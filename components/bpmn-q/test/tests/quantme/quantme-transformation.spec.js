...
describe("Test the QuantMETransformator of the QuantME extension.", function () {
  describe("Transformation of QuantME extensions", function () {
    it("should create a valid native workflow model after two transformations", async function () {
      ...
      let qrmsFirstTransformation = qrms
        .slice(0, 5)
        .concat(qrms.slice(6))
        .concat(qrmMaxCut);
      let expectedExpandedAttributes = [];
      for (let i = 0; i < qrmsFirstTransformation.length; i++) {
        let expandedAttributes = extractIsExpandedAttribute(
          qrmsFirstTransformation[i].replacement
        );
        expectedExpandedAttributes =
          expectedExpandedAttributes.concat(expandedAttributes);
      }
      ...
      chai
        .expect(expandedFirstTransformation)
        .to.deep.equal(expectedExpandedAttributes);
      ...
      for (let i = 0; i < allQrms.length; i++) {
        let expandedAttributes = extractIsExpandedAttribute(
          allQrms[i].replacement
        );
        expectedExpandedAttributes =
          expectedExpandedAttributes.concat(expandedAttributes);
      }

      const transformationResult = await startQuantmeReplacementProcess(
        result.xml,
        allQrms,
        {
          nisqAnalyzerEndpoint: config.getNisqAnalyzerEndpoint(),
      ...
      let expandedSecondTransformation = extractIsExpandedAttribute(
        transformationResult.xml
      );
      chai
        .expect(expandedSecondTransformation)
        .to.deep.equal(expectedExpandedAttributes);
...
function extractIsExpandedAttribute(xmlString) {
  ...
  for (let i = 0; i < shapeMatches.length; i++) {
    ...
    if (bpmnElementMatch && bpmnElementMatch.length > 1) {
      ...
      let positionCircuitCutting = xmlString.search(
        'circuitCuttingSubprocess id="' + bpmnElement + '"'
      );
      let positionSubProcess = xmlString.search(
        'subProcess id="' + bpmnElement + '"'
      );
