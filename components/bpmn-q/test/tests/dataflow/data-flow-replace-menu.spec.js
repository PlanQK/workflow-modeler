...
describe("Test DataFlow Replace Menu", function () {
  ...
  it("Should contain MoreOptionsEntry for TransformationTask", function () {
    ...
    const menuEntries = dataFlowReplaceMenuProvider.getPopupMenuEntries(
      taskElement
    )({});
  ...
  it("Should create no header entries for DataMapObjects", function () {
    ...
    const menuEntries = dataFlowReplaceMenuProvider.getPopupMenuHeaderEntries(
  ...
  it("Should load entries for Transformation Task Configurations", function () {
    ...
    let fetchStub = sinon
      .stub(endpoint, "fetchConfigurations")
      .callsFake(() => {
    ...
    const transformationTaskBo = bpmnFactory.create(
      dataConsts.TRANSFORMATION_TASK
    );
