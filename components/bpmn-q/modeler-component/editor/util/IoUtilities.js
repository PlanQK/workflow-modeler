...
export async function deployWorkflowToCamunda(
  ...
  //embedded:deployment:sampleEmbeddedForm.html
  //const htmlFilePath = '<form role="form" name="form"><div class="form-group"><label for="customerId-field">Customer ID</label><input required cam-variable-name="customerId" cam-variable-type="String" class="form-control"/></div><div class="form-group"><label for="amount-field">Amount</label><input cam-variable-name="amount" cam-variable-type="Double" class="form-control"/></div></form>';
  //const htmlFilePath = '<div id="form-container">  <form role="form" name="form">    <div class="form-group">      <label for="customerId-field">Customer ID</label>      <input required cam-variable-name="customerId" cam-variable-type="String" class="form-control" id="customerId" />    </div>    <div class="form-group">      <label for="amount-field">Amount</label>      <input cam-variable-name="amount" cam-variable-type="Double" class="form-control" id="amount" />    </div> <button id="submitButton" type="button">Submit</button></div></form><script cam-script type="text/form-script">  function submitForm() {     var customerId = document.getElementById("customerId").value; console.log(customerId);var apiUrl = "https://example.com/api";  var xhr = new XMLHttpRequest();  xhr.open("POST", apiUrl, true);    xhr.setRequestHeader("Content-Type", "application/json"); var data = {      customerId: customerId,      amount: parseFloat(document.getElementById("amount").value)    };  var jsonData = JSON.stringify(data);  xhr.onreadystatechange = function () {      if (xhr.readyState === 4) {        if (xhr.status === 200) {  console.log("Request successful");  } else {  console.error("Request failed with status: " + xhr.status);   }      }    };  xhr.send(jsonData);  }  document.getElementById("submitButton").addEventListener("click", submitForm);</script>';
  // redirect page
  //const htmlFilePath = '<div id="form-container">    <form role="form" name="form">      <div class="form-group">        <label for="customerId-field">Customer ID</label>        <input required cam-variable-name="customerId" cam-variable-type="String" class="form-control" id="customerId" />      </div>      <div class="form-group">        <label for="amount-field">Amount</label>        <input cam-variable-name="amount" cam-variable-type="Double" class="form-control" id="amount" />      </div>    </form>    <button id="submitButton" type="button">Submit</button>  </div>    <script>    function submitForm() {      var customerId = document.getElementById("customerId").value;      var apiUrl = "https://example.com/api";      var xhr = new XMLHttpRequest();      xhr.open("POST", apiUrl, true);      xhr.setRequestHeader("Content-Type", "application/json");      var data = {        customerId: customerId,        amount: parseFloat(document.getElementById("amount").value)      };      var jsonData = JSON.stringify(data);      xhr.onreadystatechange = function () {        if (xhr.readyState === 4) {          if (xhr.status === 200) {            console.log("Request successful"); window.open("https://example.com/redirect-page", "_blank");     } else {       window.open("https://example.com/redirect-page", "_blank");console.error("Request failed with status: " + xhr.status);          }        }      };      xhr.send(jsonData);    }      document.getElementById("submitButton").addEventListener("click", submitForm);  </script>';
  // add result field
  const htmlFilePath = '<div id="form-container">    <form role="form" name="form">        <div class="form-group"> <label for="circuit-field">Circuit</label> <a cam-file-download="circuit">Download                Circuit File</a> </div>        <div class="form-group"> <label for="processInstanceId-field">processInstanceId</label> <input                cam-variable-name="processInstanceId" cam-variable-type="Double" class="form-control"                id="processInstanceId" /> </div>        <div class="form-group" style="display: none;" id="provider-field"> <label for="selected_qpu">Selected                Provider</label> <input required cam-variable-name="selected_provider" cam-variable-type="String"                class="form-control" id="selected_provider" /> </div>        <div class="form-group" style="display: none;" id="qpu-result-field"> <label for="selected_qpu">Selected                QPU</label> <input required cam-variable-name="selected_qpu" cam-variable-type="String"                class="form-control" id="selected_qpu" /> </div>        <div class="form-group" style="display: none;" id="already_selected_field"> <input                cam-variable-name="already_selected" cam-variable-type="Boolean" class="form-control"                id="already_selected" /> </div></div><button id="submitButton" type="button">Submit</button></form></div><script>    function submitForm() {        var circuitId = "TEST";        var apiUrl = "http://localhost:5010/nisq-analyzer/implementations/";        var xhr = new XMLHttpRequest();        xhr.open("POST", apiUrl, true);        xhr.setRequestHeader("Content-Type", "application/json");        var data = { "id": "0b01949b-f948-412e-ab40-19de9b3a8843", "algorithmName": "testiod8", "implementedAlgorithm": "0a01949b-f948-412e-ab40-19de9b3a8843", "name": "ioi", "language": "OpenQASM", "sdk": "Qiskit", "fileLocation": "http://example.com/", "selectionRule": "" };        var jsonData = JSON.stringify(data);        xhr.onreadystatechange = function () {            if (xhr.readyState === 4) {                if (xhr.status === 200) {                    console.log("Request successful"); window.open("http://localhost:5009/#/algorithms", "_blank");                    document.getElementById("provider-field").value = "ibmq";                    document.getElementById("qpu-result-field").style.display = "block";                    document.getElementById("already_selected").value = true;                }                else {                    window.open("http://localhost:5009/#/algorithms", "_blank");                    document.getElementById("provider-field").style.display = "block";                    document.getElementById("qpu-result-field").style.display = "block";                    document.getElementById("already_selected").value = true;                    console.error("Request failed with status: " + xhr.status);                }            }        };        xhr.send(jsonData);    }    document.getElementById("submitButton").addEventListener("click", submitForm);      function generateRandomId(length) {    const characters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";    let randomId = "";    for (let i = 0; i < length; i++) {        const randomIndex = Math.floor(Math.random() * characters.length);        randomId += characters.charAt(randomIndex);    }    return randomId;}              </script>';
  const htmlFile = new File([htmlFilePath], "sampleEmbeddedForm.html", { type: "text/html" });
  form.append('deployment', htmlFile)
