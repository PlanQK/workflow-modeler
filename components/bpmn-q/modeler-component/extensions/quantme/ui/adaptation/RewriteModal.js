...
export default function RewriteModal({onClose, candidates}) {
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        if (i === 0) {
            ...
            candidateButtons.push(<button id={'candidate-button' + {i}} key={i} className="adaptation-tab-button active"
                                          onClick={() => openTab(i)}>Candidate {i + 1}</button>);
            candidatesHtml.push(
                <div id={'candidate-html' + {i}} key={i} hidden={false}>
                    ...
                    <h3 className='spaceAbove spaceUnder'>The following hybrid runtimes might be used for improved
                        execution:</h3>
                    <table>
                        <tbody>
                        <tr className="spaceUnder">
                            ...
                            <td align="left">
                                <button type="button" className="qwm-btn qwm-btn-secondary"
                                        onClick={() => onSubmit(i, 'Qiskit Runtime')}>Rewrite Workflow
                                </button>
                            </td>
                        </tr>
                        <tr className="spaceUnder">
                            ...
                            <td align="left">
                                <button type="button" className="qwm-btn qwm-btn-secondary"
                                        onClick={() => onSubmit(i, 'AWS Runtime')}>Rewrite Workflow
                                </button>
                            </td>
                        </tr>
                        </tbody>
            ...
            candidateButtons.push(<button id={'candidate-button' + {i}} key={i} className="adaptation-tab-button"
                                          onClick={() => openTab(i)}>Candidate {i + 1}</button>);
            candidatesHtml.push(
    ...
    return <Modal onClose={onClose}>
        ...
        <Footer>
            <div id="rewriteFormButtons">
                <button type="button" className="qwm-btn qwm-btn-secondary" onClick={() => onClose()}>Done</button>
            </div>
        </Footer>
    </Modal>;
}
