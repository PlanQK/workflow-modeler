    ...
    this.getGroups = function (element) {
        ...
        return function (groups) {
            ...
            if (is(element, consts.TRANSFORMATION_TASK)) {
                ...
                const selectedConfiguration = getTransformationTaskConfiguration(element.businessObject.get(configConsts.SELECT_CONFIGURATIONS_ID));
                if (selectedConfiguration) {

                    // add properties group for properties defined by the configuration
                    groups.splice(1, 0, createTransformationTaskConfigurationsGroup(element, injector, translate, selectedConfiguration));
                }

                // add entries for the parameters attribute of a transformation task
                groups.push(createTransformationTaskGroup(element, injector, translate));
            }
            ...
            if (is(element, consts.TRANSFORMATION_ASSOCIATION)) {
                groups.push(createTransformationAssociationGroup(element, injector, translate));
            }
