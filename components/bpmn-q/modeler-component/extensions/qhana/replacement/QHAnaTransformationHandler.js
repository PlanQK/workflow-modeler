...
export async function startQHAnaReplacementProcess(xml) {
  ...
  const qhanaServiceTasks = getAllElementsInProcess(
    rootProcess,
    elementRegistry,
    consts.QHANA_SERVICE_TASK
  );
  console.log(
    "Process contains " +
      qhanaServiceTasks.length +
      " QHAna service tasks to replace..."
  );

  // get all QHAna Service Step Tasks from the process
  const qhanaServiceStepTasks = getAllElementsInProcess(
    rootProcess,
    elementRegistry,
    consts.QHANA_SERVICE_STEP_TASK
  );
  console.log(
    "Process contains " +
      qhanaServiceStepTasks.length +
      " QHAna service step tasks to replace..."
  );

  // skip transformation if no QHAna service tasks and no QHAna service step tasks exist in the process
  if (
    (!qhanaServiceTasks || !qhanaServiceTasks.length) &&
    (!qhanaServiceStepTasks || !qhanaServiceStepTasks.length)
  ) {
  ...
  for (let qhanaServiceTask of qhanaServiceTasks) {
    ...
    console.log(
      "Replacing QHAna service task with id %s ",
      qhanaServiceTask.element.id
    );
    replacementSuccess = await replaceQHAnaServiceTaskByServiceTask(
      definitions,
      qhanaServiceTask.element,
      qhanaServiceTask.parent,
      modeler
    );

    if (!replacementSuccess) {
      console.log(
        "Replacement of QHAna service task with id " +
          qhanaServiceTask.element.id +
  ...
  for (let qhanaServiceTask of qhanaServiceStepTasks) {
    let replacementSuccess = false;
    console.log(
      "Replacing QHAna service step task with id %s ",
      qhanaServiceTask.element.id
    );
    replacementSuccess = await replaceQHAnaServiceStepTaskByServiceTask(
      definitions,
      qhanaServiceTask.element,
      qhanaServiceTask.parent,
      modeler
    ...
    if (!replacementSuccess) {
      console.log(
        "Replacement of QHAna service step task with id " +
          qhanaServiceTask.element.id +
          " failed. Aborting process!"
      );
      return {
 ...
 */
async function replaceQHAnaServiceTaskByServiceTask(
  definitions,
  qhanaServiceTask,
  parentProcess,
  modeler
  ...
  if (result.success && result.element) {
    ...
    addCamundaInputParameter(
      newElement.businessObject,
      "qhanaName",
      qhanaServiceTask.qhanaName,
      bpmnFactory
    );
    addCamundaInputParameter(
      newElement.businessObject,
      "qhanaDescription",
      qhanaServiceTask.qhanaDescription,
      bpmnFactory
    );
  }
 ...
 * @return {Promise<boolean>}
 */
async function replaceQHAnaServiceStepTaskByServiceTask(
  definitions,
  qhanaServiceTask,
  parentProcess,
  modeler
