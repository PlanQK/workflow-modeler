    ...
    collapseSurroundingSubProcess(element) {
        ...
        if (surroundingSubProcess) {
            ...
            if (parent === surroundingSubProcess.element) {
                ...
                if (JSON.stringify(currentBounds) === JSON.stringify(surroundingSubProcess.newBounds)) {
                    const event = {
                        context: {
                            shape: parent,
                            newBounds: surroundingSubProcess.oldBounds,
                            hints: {},
                        },
                    };
    ...
    expandSurroundingSubProcess(element, boundingBox) {
        ...
        if (!element.surroundingSubProcess) {
            ...
            if (parent.type === "bpmn:SubProcess") {
                ...
                if (oldBounds.x + NODE_SHIFT_MARGIN > boundingBox.left) {
                    newBounds.x = boundingBox.left;
                    needsResize = true;
                }
                if (oldBounds.y + NODE_SHIFT_MARGIN > boundingBox.top) {
                    newBounds.y = boundingBox.top;
                    needsResize = true;
                }
                if (oldBounds.x + oldBounds.width < boundingBox.right + NODE_SHIFT_MARGIN) {
                    newBounds.width = boundingBox.right - oldBounds.x;
                    needsResize = true;
                }
                if (oldBounds.y + oldBounds.height < boundingBox.bottom + NODE_SHIFT_MARGIN) {
                    newBounds.height = boundingBox.bottom - oldBounds.y;
                    needsResize = true;
                }
