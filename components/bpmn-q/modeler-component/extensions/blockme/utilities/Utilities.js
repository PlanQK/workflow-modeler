...
import { BlockMEProps } from "../modeling/properties-provider/BlockMEPropertiesProvider";
import { getQRMs, updateQRMs } from "../qrm-manager";
import { uploadMultipleToGitHub } from "../qrm-manager/git-handler";

/**
 * Check if the given task is a BlockME task
 *
 * @param task the task to check
 * @returns true if the passed task is a BlockME task, false otherwise
 */
export function isBlockMETask(task) {
  return task.$type.startsWith("blockme:");
}
...
export async function createDetector(elementToCopy) {
  ...
  const propertiesToCopy = BlockMEProps(elementToCopy);
