  ...
  if (flowElements) {
    for (let i = 0; i < flowElements.length; i++) {
      if (isFlowLikeElement(flowElements[i].$type)) {
        ...
        if (["bpmn:SubProcess"].includes(flowElements[i].$type)) {
...
function layoutBoundaryEvents(modeling, elementRegistry) {
  ...
  for (let boundaryEvent of elementRegistry.getAll()) {
    if (boundaryEvent.type === "bpmn:BoundaryEvent") {
      ...
      let offset = 65 + attachedToElementBoundaries.length * (parseInt(boundaryEventBounds.width) + parseInt(BOUNDARY_EVENT_MARGIN));
      let to_move_x = bottomOfAttached - offset;
      let to_move_y = attachedToBounds.y - boundaryEventBounds.y +
        attachedToBounds.height - boundaryEventBounds.height / 2;
